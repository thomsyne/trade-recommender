...
The provider endpoints do not give a trustworthy release timestamp for these
...
CPI and employment are deliberately absent. They should only be enabled after
current official identifiers, revision behavior, and publication-time
semantics are verified. The discontinued ONS `cpih01` beta dataset is not used.
...
Raw retrievals, representations, macro observations, and discrepancies reject
...
Seeding creates four six-hour feed schedules and four daily macro schedules:
...
The Research tab shows freshness, hashes, timestamp quality, retention state,
and a provider-comparison matrix. Trading Economics is only an unmeasured
calendar candidate; no paid calendar provider has been selected.
