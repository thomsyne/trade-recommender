...
def rights(source_id=1, processor="anthropic"):
    return {
        "version": VERSION,
        "source_id": source_id,
        "processor": processor,
...
class EvidenceSemanticsTests(SimpleTestCase):
    ...
    def test_cap_after_rights_relevance_dedup_and_stable_order(self):
        ...
        self.assertEqual(p["excluded_counts"], {"rights_blocked": 1, "irrelevant": 1, "cap": 2})
...
class EvidencePersistenceTests(TestCase):
    def setUp(self):
        ...
        self.local = review_rights(self.policy.source, rights(self.policy.source_id, "local"))
        self.external = review_rights(self.policy.source, rights(self.policy.source_id))
    ...
    def test_incident_dedupe_retains_distinct_discrepancies(self):
        ...
        self.assertEqual(EvidenceConflict.objects.count(), 2)
