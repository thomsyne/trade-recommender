"""Gate 7A: provider-observed acquisition canary activation.

Additive activation boundary allowing exactly attempt 1 of the single
governed replacement acquisition canary chunk (AUD_USD H1, discovery
ordinal 2) and nothing else. Every other replacement chunk, every retry,
and every post-terminal attempt is rejected inside PostgreSQL. Legacy v1
acquisition rules are preserved byte-for-byte and restored verbatim by an
empty reversal."""
...
# migration 0019, captured from a catalog migrated exactly through 0019
# with the same fingerprint queries migration 0014's preflight uses.
REQUIRED_0019_FUNCTIONS = {
    ...
    "market_governed_candle_reject_mutation": ("", "4164da30f02bc83aa5b7945c5231ffd3"),
    ...
    "market_ingestion_run_enforce": ("", "5527e44615986366aeebb6e943918769"),
    ...
    "market_validate_historical_attempt": ("", "46a0c012e6bd1a63bae6824a45543711"),
...
# Verbatim pg_proc.prosrc of the three migration 0010 acquisition
# governance bodies this activation replaces, restored byte-identically by
# the empty reversal.
HISTORICAL_ATTEMPT_0019_PROSRC = r"""
...
GOVERNED_CANDLE_0019_PROSRC = r"""
        ...
        BEGIN
          ...
          END IF; RETURN NEW;
        END """
HISTORICAL_ATTEMPT_GATE7A_PROSRC = r"""
        DECLARE chunk record; run record; plan_source bigint; next_number integer;
        BEGIN
          SELECT * INTO STRICT chunk FROM market_historicalingestionchunk WHERE id=NEW.chunk_id FOR UPDATE;
          SELECT * INTO STRICT run FROM market_ingestionrun WHERE id=NEW.ingestion_run_id;
          ...
          IF chunk.data_contract_sha256 IS NOT NULL THEN
            PERFORM market_validate_acquisition_canary(NEW.chunk_id,
              NEW.attempt_number, NEW.idempotency_key, NEW.ingestion_run_id);
          END IF;
          RETURN NEW;
        END """
INGESTION_RUN_GATE7A_PROSRC = r"""
        DECLARE historical boolean; has_attempt boolean;
        BEGIN
          IF TG_OP = 'INSERT' THEN
            historical := NEW.dataset_version_id IS NOT NULL AND EXISTS
            ...
            IF NEW.dataset_version_id IS NOT NULL AND EXISTS (
              ...
              IF NOT EXISTS (SELECT 1 FROM market_historicalingestionchunk cc
                    WHERE cc.dataset_version_id=NEW.dataset_version_id
                      AND cc.logical_key='e0a19ed9db1707420233af059f7c9f8d84fe87afe1ad59e7ab2e8b195121fd3c'
                 ...
                 OR EXISTS (SELECT 1 FROM market_historicalingestionattempt aa
                      JOIN market_historicalingestionchunk cc2 ON cc2.id=aa.chunk_id
                      WHERE cc2.data_contract_sha256 IS NOT NULL)
              THEN RAISE EXCEPTION
                'acquisition canary activation rejects this ingestion run'; END IF;
            END IF;
            RETURN NEW;
          END IF;
          has_attempt := EXISTS (SELECT 1 FROM market_historicalingestionattempt
          ...
          RETURN NEW;
        END """
GOVERNED_CANDLE_GATE7A_PROSRC = r"""
        DECLARE chunk record;
        BEGIN
          IF TG_OP='DELETE' THEN IF OLD.dataset_version_id IS NOT NULL THEN
            RAISE EXCEPTION 'governed dataset candles are append-only'; END IF; RETURN OLD; END IF;
          ...
          IF EXISTS (SELECT 1 FROM market_datasetversion WHERE id=NEW.dataset_version_id
            ...
            IF chunk.id IS NOT NULL AND chunk.data_contract_sha256 IS NOT NULL THEN
              IF chunk.dataset_version_id<>NEW.dataset_version_id
                 ...
                 OR NOT NEW.complete OR NEW.timestamp<chunk.requested_from
                 OR NEW.timestamp>=chunk.requested_to
                 OR chunk.logical_key<>'e0a19ed9db1707420233af059f7c9f8d84fe87afe1ad59e7ab2e8b195121fd3c'
...
ACQUISITION_CANARY_PROSRC = r"""
        ...
        BEGIN
          PERFORM pg_advisory_xact_lock(hashtextextended('gate7a-acquisition-canary',0));
          ...
          IF plan.data_contract_id IS NULL THEN
            RAISE EXCEPTION 'acquisition canary activation rejects this attempt';
          END IF;
          ...
          SELECT i.code INTO STRICT instrument_code FROM market_instrument i
            WHERE i.id=chunk.instrument_id;
          IF contract.sha256<>'60b603f26662bfc8faa4373177690bc0ae23820b914815f47c11d8367c07f7bf'
             ...
             OR chunk.expected_observation_count<>2932
             ...
             OR instrument_code<>'AUD_USD'
             OR inventory.semantic_inventory_sha256<>chunk.semantic_inventory_sha256
             OR inventory.observation_count<>2932
             OR observation_rows<>2932
             OR discovery_chunk.ordinal<>2
             ...
             OR EXISTS (SELECT 1 FROM market_historicalingestionattempt a
                        JOIN market_historicalingestionchunk c2 ON c2.id=a.chunk_id
             ...
             OR EXISTS (SELECT 1 FROM market_ingestionrun r2
                        ...
                        WHERE r2.status='running' AND r2.id<>new_run_id)
             OR EXISTS (SELECT 1 FROM market_datasetregistration
                        WHERE dataset_version_id=chunk.dataset_version_id)
          THEN RAISE EXCEPTION 'acquisition canary activation rejects this attempt'; END IF;
        END """
...
def preflight_acquisition_canary(apps, schema_editor):
    if schema_editor.connection.vendor != "postgresql":
        return
    governance = _governance()
    problems = []
    with schema_editor.connection.cursor() as cursor:
        cursor.execute(governance.FUNCTION_FINGERPRINT_SQL, [list(REQUIRED_0019_FUNCTIONS)])
        found_functions = {}
        ...
        for name, expected in sorted(REQUIRED_0019_FUNCTIONS.items()):
            candidates = found_functions.get(name, [])
            if not candidates:
                problems.append(f"required 0019 function {name} is missing")
            elif len(candidates) > 1:
                problems.append(f"required 0019 function {name} has ambiguous overloads")
            elif candidates[0] != tuple(expected):
                problems.append(f"required 0019 function {name} does not match its 0019 definition")
        cursor.execute(
        ...
        for (name,) in cursor.fetchall():
            if name not in REQUIRED_0019_FUNCTIONS:
                problems.append(f"unexpected function {name} outside the 0019 catalog")
        ...
        for (table, name), expected in sorted(REQUIRED_0019_TRIGGERS.items()):
            candidates = found_triggers.get((table, name), [])
            if not candidates:
                problems.append(f"required 0019 trigger {name} on {table} is missing")
            elif len(candidates) > 1:
                problems.append(f"required 0019 trigger {name} on {table} is ambiguous")
            elif candidates[0] != expected:
                problems.append(
                    f"required 0019 trigger {name} on {table} does not match its 0019 definition"
                )
        for table, name in sorted(found_triggers):
            if table.startswith("market_") and (table, name) not in REQUIRED_0019_TRIGGERS:
                problems.append(f"unexpected trigger {name} on {table} outside the 0019 catalog")
        cursor.execute(
        ...
        if cursor.fetchone()[0] != 0:
            problems.append("replacement acquisition attempts already exist")
        cursor.execute(
            """SELECT count(*) FROM market_candle cd
               JOIN market_datasetversion dv ON dv.id=cd.dataset_version_id
               WHERE dv.data_contract_sha256 IS NOT NULL"""
        )
        if cursor.fetchone()[0] != 0:
            problems.append("replacement candles already exist")
        cursor.execute(
            """SELECT count(*) FROM market_ingestionmanifest m
               JOIN market_datasetversion dv ON dv.id=m.dataset_version_id
               WHERE dv.data_contract_sha256 IS NOT NULL"""
        )
        if cursor.fetchone()[0] != 0:
            problems.append("replacement ingestion manifests already exist")
    if problems:
        raise RuntimeError(
            "migration 0020 preflight rejected the current catalog: " + "; ".join(problems)
        )


def install_acquisition_canary(apps, schema_editor):
    if schema_editor.connection.vendor != "postgresql":
        return
    _execute(
        schema_editor,
        "CREATE FUNCTION market_validate_acquisition_canary("
        "attempt_chunk_id bigint, new_attempt_number integer, "
        "new_idempotency_key text, new_run_id bigint) RETURNS void AS $governed$"
        + ACQUISITION_CANARY_PROSRC
        + "$governed$ LANGUAGE plpgsql;",
    )
    _execute(
        schema_editor,
        "CREATE OR REPLACE FUNCTION market_validate_historical_attempt() "
        "RETURNS trigger AS $governed$" + HISTORICAL_ATTEMPT_GATE7A_PROSRC + "$governed$ "
        "LANGUAGE plpgsql;",
    )
    _execute(
        schema_editor,
        "CREATE OR REPLACE FUNCTION market_ingestion_run_enforce() "
        "RETURNS trigger AS $governed$" + INGESTION_RUN_GATE7A_PROSRC + "$governed$ "
        "LANGUAGE plpgsql;",
    )
    _execute(
        schema_editor,
        ...
        cursor.execute(
            """SELECT
               ...
               + (SELECT count(*) FROM market_ingestionmanifest m
                  JOIN market_datasetversion dv ON dv.id=m.dataset_version_id
                  WHERE dv.data_contract_sha256 IS NOT NULL)
               + (SELECT count(*) FROM market_ingestionrun r
                  JOIN market_datasetversion dv ON dv.id=r.dataset_version_id
                  WHERE dv.data_contract_sha256 IS NOT NULL)"""
        )
        if cursor.fetchone()[0] != 0:
            raise RuntimeError("replacement acquisition evidence prohibits gate7a reversal")
    _execute(
        schema_editor,
        "CREATE OR REPLACE FUNCTION market_validate_historical_attempt() "
        "RETURNS trigger AS $governed$" + HISTORICAL_ATTEMPT_0019_PROSRC + "$governed$ "
        "LANGUAGE plpgsql;",
    )
    _execute(
        schema_editor,
        "CREATE OR REPLACE FUNCTION market_ingestion_run_enforce() "
        "RETURNS trigger AS $governed$" + INGESTION_RUN_0019_PROSRC + "$governed$ "
        "LANGUAGE plpgsql;",
    )
    _execute(
        schema_editor,
        "CREATE OR REPLACE FUNCTION market_governed_candle_reject_mutation() "
        "RETURNS trigger AS $governed$" + GOVERNED_CANDLE_0019_PROSRC + "$governed$ "
        "LANGUAGE plpgsql;",
    )
    _execute(
        schema_editor,
...
class Migration(migrations.Migration):
    ...
    operations = [
        migrations.RunPython(preflight_acquisition_canary, migrations.RunPython.noop),
        migrations.RunPython(install_acquisition_canary, remove_acquisition_canary),
    ]
