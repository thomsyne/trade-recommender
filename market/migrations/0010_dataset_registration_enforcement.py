...
def preflight(apps, schema_editor):
    """Fail closed over all evidence written while 0009 lacked database enforcement."""
    ...
    historical_datasets = Dataset.objects.filter(manifest__has_key="historical_plan_sha256")
    evidence_exists = (
        Plan.objects.exists()
        or Chunk.objects.exists()
        or Attempt.objects.exists()
        or historical_datasets.exists()
        or IngestionRun.objects.filter(dataset_version__in=historical_datasets).exists()
        or Manifest.objects.filter(dataset_version__in=historical_datasets).exists()
        or Candle.objects.filter(dataset_version__in=historical_datasets).exists()
        or Conflict.objects.filter(dataset_version__in=historical_datasets).exists()
        or Incident.objects.filter(dataset_version__in=historical_datasets).exists()
        or AuditEvent.objects.filter(event_type__startswith="market.historical_attempt_").exists()
    )
    if evidence_exists:
        raise RuntimeError(
            "partial 0009 contains Phase 2B evidence written without complete enforcement"
        )
...
def create_governance_triggers(apps, schema_editor):
    ...
    schema_editor.execute(
        ...
        CREATE FUNCTION market_historical_manifest_valid(
          manifest_payload jsonb, manifest_dataset_id bigint, manifest_run_id bigint
        ) RETURNS boolean AS $$
        DECLARE lineage record; request_item jsonb;
        BEGIN
          SELECT a.attempt_number,a.idempotency_key,c.logical_key,c.dataset_version_id,
                 ...
                 c.canonical_request,p.source_id,i.code,r.status,r.dataset_version_id AS run_dataset_id,
                 r.source_id AS run_source_id,r.instrument_id AS run_instrument_id,
                 r.granularity AS run_granularity,r.requested_from AS run_from,
                 r.requested_to AS run_to,r.parameters,r.request_manifest_hash
            INTO STRICT lineage
          ...
          RETURN coalesce(lineage.dataset_version_id=manifest_dataset_id
             ...
             AND lineage.run_from=lineage.requested_from
             AND lineage.run_to=lineage.requested_to
             ...
             AND jsonb_typeof(manifest_payload->'instrument')='string'
             AND manifest_payload->>'instrument'=lineage.code
             AND jsonb_typeof(manifest_payload->'granularity')='string'
             AND manifest_payload->>'granularity'=lineage.granularity
             AND jsonb_typeof(manifest_payload->'from')='string'
             AND (manifest_payload->>'from')::timestamptz=lineage.requested_from
             AND jsonb_typeof(manifest_payload->'to')='string'
             ...
             AND jsonb_typeof(request_item)='object'
             AND request_item->'status'='200'::jsonb, false);
        ...
        BEGIN
          IF TG_OP='DELETE' THEN
            IF EXISTS (SELECT 1 FROM market_datasetversion WHERE id=OLD.dataset_version_id
                       AND manifest ? 'historical_plan_sha256')
            THEN RAISE EXCEPTION 'historical ingestion manifests are append-only'; END IF;
            RETURN OLD;
          END IF;
          IF TG_OP='UPDATE' AND (EXISTS
             (SELECT 1 FROM market_datasetversion WHERE id=OLD.dataset_version_id
              AND manifest ? 'historical_plan_sha256') OR EXISTS
             (SELECT 1 FROM market_datasetversion WHERE id=NEW.dataset_version_id
              AND manifest ? 'historical_plan_sha256'))
          THEN RAISE EXCEPTION 'historical ingestion manifests are append-only'; END IF;
          ...
          IF EXISTS (SELECT 1 FROM market_datasetversion WHERE id=NEW.dataset_version_id
                     AND manifest ? 'historical_plan_sha256') AND
             (NEW.sha256<>market_sha256(NEW.payload) OR NOT market_historical_manifest_valid(
                NEW.payload,NEW.dataset_version_id,NEW.ingestion_run_id))
          THEN RAISE EXCEPTION 'historical manifest conflicts with canonical attempt semantics'; END IF;
          RETURN NEW;
        END $$ LANGUAGE plpgsql;
        CREATE TRIGGER market_ingestion_manifest_historical_validate BEFORE INSERT OR UPDATE OR DELETE ON market_ingestionmanifest
...
def drop_governance_triggers(apps, schema_editor):
    ...
    schema_editor.execute(
        ...
        DROP FUNCTION IF EXISTS market_historical_manifest_valid(jsonb,bigint,bigint);
