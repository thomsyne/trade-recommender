...
class FreshnessTests(TestCase):
    ...
    def test_expected_completion_respects_weekend_and_new_york_close(self):
        saturday = datetime(2026, 9, 12, 12, tzinfo=UTC)
        self.assertEqual(
            latest_expected_completion(saturday, "D"), datetime(2026, 9, 11, 21, tzinfo=UTC)
        )
        self.assertEqual(
            latest_expected_completion(saturday, "W"), datetime(2026, 9, 11, 21, tzinfo=UTC)
        )
        self.assertEqual(
            latest_expected_completion(saturday, "H1"), datetime(2026, 9, 11, 21, tzinfo=UTC)
        )
        self.assertEqual(
            latest_expected_completion(saturday, "H4"), datetime(2026, 9, 11, 21, tzinfo=UTC)
        )
        wednesday = datetime(2026, 9, 9, 15, 30, tzinfo=UTC)
        self.assertEqual(
            latest_expected_completion(wednesday, "H1"), datetime(2026, 9, 9, 15, tzinfo=UTC)
        )
        self.assertEqual(
            latest_expected_completion(wednesday, "H4"), datetime(2026, 9, 9, 13, tzinfo=UTC)
        )
        self.assertEqual(
            latest_expected_completion(wednesday, "D"), datetime(2026, 9, 8, 21, tzinfo=UTC)
        )

    def test_weekend_daily_candle_is_fresh_and_next_candle_is_sunday_open(self):
        thursday_start = datetime(2026, 9, 10, 21, tzinfo=UTC)
        self.store("D", [thursday_start])
        job = self.job("D", 86_400, datetime(2026, 9, 12, 22, tzinfo=UTC))

        row = series_freshness(
            self.instrument, "D", now=datetime(2026, 9, 12, 12, tzinfo=UTC), job=job
        )

        self.assertEqual(row.latest_interval_start, thursday_start)
        self.assertEqual(row.latest_completion, datetime(2026, 9, 11, 21, tzinfo=UTC))
        self.assertEqual(row.next_interval_start, datetime(2026, 9, 13, 21, tzinfo=UTC))
        self.assertEqual(row.next_completion, datetime(2026, 9, 14, 21, tzinfo=UTC))
    ...
    def test_weekly_freshness_follows_friday_close_and_polling_interval(self):
        week_start = datetime(2026, 9, 4, 21, tzinfo=UTC)
        self.store("W", [week_start])
        job = self.job("W", 604_800, datetime(2026, 9, 15, tzinfo=UTC))

        fresh = series_freshness(
            self.instrument, "W", now=datetime(2026, 9, 12, tzinfo=UTC), job=job
        )
        tolerated = series_freshness(
            self.instrument, "W", now=datetime(2026, 9, 20, tzinfo=UTC), job=job
        )
        stale = series_freshness(
            self.instrument, "W", now=datetime(2026, 9, 26, 12, tzinfo=UTC), job=job
        )

        self.assertEqual(fresh.latest_completion, datetime(2026, 9, 11, 21, tzinfo=UTC))
    ...
    def test_interval_start_completion_and_retrieval_are_reported_separately(self):
        ...
        row = series_freshness(self.instrument, "H1", now=datetime(2026, 9, 9, 14, 30, tzinfo=UTC))
    ...
    def test_weekday_hourly_series_goes_stale_after_interval_and_grace(self):
        self.store("H1", [datetime(2026, 9, 9, 12, tzinfo=UTC)])
        job = self.job("H1", 3_600, datetime(2026, 9, 9, 16, tzinfo=UTC))
        ...
        within = series_freshness(
            self.instrument, "H1", now=datetime(2026, 9, 9, 14, 50, tzinfo=UTC), job=job
        )
        stale = series_freshness(
            self.instrument, "H1", now=datetime(2026, 9, 9, 15, 30, tzinfo=UTC), job=job
        )
    ...
    def test_missing_series_is_reported_as_no_data(self):
        row = series_freshness(self.instrument, "H4", now=datetime(2026, 9, 9, 15, tzinfo=UTC))
